...
class Skouerr_CLI
{
    ...
    public function list_command()
    {
        ...
        try {
            ...
            $headers = array('Nom de la commande', 'Titre', 'Description');

            WP_CLI::line('List des commandes utilisées par le plugin starter-toolbox');
            self::output_table($data, $headers);
        } catch (Exception $e) {
            WP_CLI::error('Une erreur est survenue lors de la récupération des commandes: ' . $e->getMessage());
        }
    }
    ...
    private static function format_commands_data($cli_name, $commands)
    {
        ...
        foreach ($commands as $command) {
            $data[] = array(
                'Nom de la commande' => $cli_name . ' ' . $command['name'],
                'Titre' => $command['title'],
